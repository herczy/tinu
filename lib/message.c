...
gboolean
msg_serialize(const Message *self, GByteArray *dest)
{
  gint i;
  gboolean res;

  t_putv(dest, self->m_priority);
  t_puts(dest, self->m_message);
  t_putv(dest, self->m_tag_count);

  for (i = 0; i < self->m_tag_count; i++)
    {
      t_puts(dest, self->m_tags[i]->m_tag);
      t_puts(dest, self->m_tags[i]->m_value);
    }
...
Message *
msg_deserialize(const GByteArray *data)
{
  gint pos = 0;
  gint i;
  MessageTag *tag = NULL;
  Message *self = t_new(Message, 1);

  if (!t_getv(data, self->m_priority) ||
      !t_gets(data, &self->m_message, &pos) ||
      !t_getv(data, self->m_tag_count))
    goto error;

  if (self->m_tag_count> 0)
    {
      self->m_tags = t_new(MessageTag *, self->m_tag_count);
    
      for (i = 0; i < self->m_tag_count; i++)
        {
          tag = t_new(MessageTag, 1);
          if (!t_gets(data, &tag->m_tag, &pos) || !t_gets(data, &tag->m_value, &pos))
            goto error;
